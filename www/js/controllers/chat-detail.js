...
function($scope, $rootScope, $state, $stateParams, MockService,
	$ionicActionSheet, $ionicPopup, $ionicScrollDelegate, $timeout, $interval, $ionicLoading, messageService, artistService, customerService, $ionicScrollDelegate, Pubnub) {
		...
		function getCustomerProfile(){
			customerService.getCustomerById($stateParams.customerId)
			.then(function(results) {
				// Handle the result
				console.log(results);
				$scope.currentCustomerProfile = results[0];

				$scope.toUser.username = results[0].get('firstName') + ' ' + results[0].get('lastName');
				$scope.toUser.pic = results[0].get('avatar');
				getMessagesApi();
				$ionicLoading.hide();
			}, function(err) {
				// Error occurred
		...
		function getArtistById(id){
			...
			.then(function(results) {
				...
				$scope.user.username = $scope.profile.firstName + ' ' + $scope.profile.lastName,
				$scope.user.pic = $scope.profile.avatar;
				getCustomerProfile();
...
me.getMockMessage = function(username) {
	return {
		userId: '534b8e5aaa5e7afc1b23e69b',
		date: new Date(),
		text: 'Hi ' + username + ', Welcome and thank you for trying out Blush!\n\nIn this feature you can inquire for services, upload photos for references, or just try to talk with the artists :)'
	};
}
